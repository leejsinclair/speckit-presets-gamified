...
argument-hint: "(no arguments needed — assesses the active feature's spec.md against its story.md)"
...
metadata:
  ...
  source: "extensions/questmaster/commands/speckit.questmaster.check-spec.md"
...
The Specification Integrity assessment (FR-015, FR-017–FR-021, FR-037–FR-040,
contracts/quest-check-spec.md). Invoked automatically by the `after_specify` hook, and
independently re-runnable on demand. Bands `spec.md` against `story.md` across 7 dimensions in an
**independent context**, applies the six-way Drift Classification to every requirement/scope
item/assumption, and reports decision-first.

**Trigger awareness**: determine whether this invocation is hook-triggered (fired automatically
right after `/speckit-specify` completed, as this command's `after_specify` registration) or an
explicit/manual invocation (the developer ran `/speckit-questmaster-check-spec` directly). This
distinction matters for Step 2's silent-exit behavior below.
...
### Step 1 — Relocate a pending story, if any (FR-029)

If `.specify/extensions/questmaster/pending-story.md` exists (project root — check with
`qm-pending.sh exists <project-root>`) **and** the active feature directory has no `story.md`
yet, relocate it now:

```sh
sh qm-pending.sh relocate <project-root> <feature-dir>
```

This moves the pending story into `<feature-dir>/story.md` before continuing. This is the one
place the pending-story mechanism resolves — it never required touching `speckit-specify` itself
(research.md §2/§3).

### Step 2 — Resolve the active feature directory (FR-025, FR-026)

Resolve the active feature directory from `.specify/feature.json`. If it has **no `story.md`**
(and Step 1 found no pending story to relocate):
- **Hook-triggered invocation**: exit immediately. Write nothing, print nothing — a non-adopting
  project's `/speckit-specify` completion report must be completely unaffected.
...
### Step 3 — Require spec.md

Require `spec.md` to exist in the same feature directory (it always will when hook-triggered,
since `/speckit-specify` just created it).

### Step 4 — Compute source digests (FR-040)
...
If a previous `<feature-dir>/spec-integrity.md` exists, read its `**Source digests**:` line and
compare: if either digest no longer matches, state in the new report which upstream artifact
changed and that the previous report's findings were based on a different version. Remember: the
Questmaster Record itself is excluded from what these digests cover, so Questmaster's own writes
(this very command's Step 7 below, or `/speckit-questmaster-story` recording a Decision) never
trigger a false "changed since last report."

### Step 5 — Read outstanding accepted risk (FR-039)

```sh
sh qm-record.sh list-decisions <feature-dir>/story.md
```

For every entry with `"status": "outstanding"`, this run must judge — **as part of the
independent assessment in Step 6**, not from this authoring context — whether the condition that
prompted the acceptance still holds:
- **still holds**: restate it in the report (accepted date + the developer's own reason), ahead
  of this run's own findings.
- **no longer holds**: it MAY be auto-resolved, but only with specific artifact content cited as
  evidence. Record the auto-resolution with:
  ```sh
  sh qm-record.sh resolve-decision <feature-dir>/story.md "<a substring matching the narrative>" questmaster <today> "<specific evidence>"
  ```
  This call **refuses** (non-zero exit) if evidence is empty — never attempt to work around that
  refusal by inventing a placeholder string; if you don't have specific evidence, the risk stays
  outstanding. An auto-resolution is attributed to `questmaster`, never `developer`, is **not** a
  Judgment Ledger entry, and must be reported in this run's output as reopenable. Absence of an
  objection in `spec.md` is never evidence.

### Step 6 — Independent assessment (FR-037)

**This is the step that MUST run with no access to whatever conversation authored `spec.md`.**
Invoke Claude Code's **Agent tool** (a fresh, independent agent — not a `fork`, which would
inherit this conversation) with a prompt containing **only**:
- the full content of `<feature-dir>/story.md` and `<feature-dir>/spec.md`,
- the resolved `specification_integrity_rubric` (from `qm-config.sh`),
- `story.md`'s Decisions subsection (for the still-holds/auto-resolve judgment from Step 5), and
- the seven-dimension rubric's band anchors,

asking it to:
1. Band all 7 `specification_integrity_rubric` dimensions (`story_fidelity`,
   `requirement_completeness`, `requirement_testability`, `scope_discipline`, `traceability`,
   `handling_of_ambiguity`, `internal_consistency`), each with cited evidence (FR-036).
2. For every requirement, scope item, and assumption in `spec.md`, assign exactly one Drift
   Classification against `story.md`: `PRESERVED`, `REFINED`, `CLARIFIED`, `DISCOVERED`,
   `ACCEPTED_SCOPE_CHANGE`, or `UNJUSTIFIED_DRIFT` (FR-017). It MUST consult `story.md`'s
   Decisions first and only use `ACCEPTED_SCOPE_CHANGE` where a matching recorded acceptance
   exists — otherwise an out-of-story-scope element is `UNJUSTIFIED_DRIFT` (FR-019). It MUST
   NOT classify content `UNJUSTIFIED_DRIFT` solely for not being verbatim in the story —
   reasonable elaboration is `REFINED`/`CLARIFIED` (FR-020) — and MUST compare by underlying
   intent, not by matching section titles or literal wording.
3. Judge the Step 5 outstanding-risk still-holds/no-longer-holds question, citing specific
   evidence for any "no longer holds" verdict.
4. For every `DISCOVERED`/`ACCEPTED_SCOPE_CHANGE`/`UNJUSTIFIED_DRIFT` element, produce a full
   ...
   evidence, severity, and the specific developer question (FR-018).

Ask for the result as structured JSON matching `qm-report.sh`'s report-spec schema fields
(`dimension_bands`, `drift_counts`, `findings`, `appendix`) plus the outstanding-risk verdicts.

**Determine the assessment context label**: if the Agent tool (or an equivalent independent-
context mechanism) was actually available and used, the context is `INDEPENDENT`. If no such
mechanism was available in this environment and the assessment had to run inline in this same
conversation instead, the context is `SELF-ASSESSED` — this label is **not** suppressible by any
configuration and MUST appear in both the response and the persisted report.

### Step 7 — Assemble and write the report (FR-022, FR-031, FR-038)

Build a report-spec JSON from the subagent's output (Step 6) plus Steps 4-5's digests and
outstanding-risk data, then render it:

```sh
sh qm-report.sh <report-spec.json>
```

Write the result to `<feature-dir>/spec-integrity.md` (overwriting any previous version), and
present the **same content, in the same order**, in your response. Never auto-modify, block, or
reject `spec.md` on the basis of any finding — every Questmaster output here is advisory only.

### Step 8 — Elicit a response to the top decision (FR-043)

**This step is direct developer interaction, not subject to Step 6's independence requirement.**

Determine whether this step applies at all, deterministically, from the same `decisions` array
you passed to `qm-report.sh` in Step 7:

```sh
sh qm-worklist.sh <decisions.json>
```

If `.required` is `false` (an empty worklist — Step 7 already said "No decision required."),
**skip this step entirely — ask nothing.** Otherwise `.top` names the highest-severity item to
put to the developer now.

Ask the developer to resolve, accept, or explicitly defer that top item.
- **Resolve**: record their stated resolution.
- **Accept**: requires a justification the developer types themselves (FR-014) — the same rule
  as story-stage acceptance: a menu choice, generated text, or silence does not count. Record it
  as a Decision (`qm-record.sh add-decision <feature-dir>/spec.md ...`, status `outstanding`).
- **Defer**: record the deferral explicitly as a deferral, not as an absence.
- **No response given**: record it as unanswered, carried forward per FR-039 (it will be
  restated at the Plan Integrity stage as a still-open item, via Step 5's mechanism there).

Whatever the outcome, add a Judgment Ledger entry to `spec.md` via
`qm-record.sh add-ledger <feature-dir>/spec.md <today> spec <kind> "<verbatim developer response>" "<effect>"`
(`kind` is `asserted_fact` for a resolution/clarification, or `risk_named`/`correction` as fits —
choose whichever kind best describes what the developer actually contributed).

**Never block** `/speckit-plan` on the answer to this step, whatever it is.

## Output

- `<feature-dir>/spec-integrity.md` (written only when a `story.md` exists or was relocated)
- `spec.md`'s Questmaster Record updated with the developer's response to the top Decision
  Worklist item (Step 8), or nothing at all when the worklist was empty
- Response, in order: Decision Worklist, Outstanding Accepted Risk (including any auto-resolved
  this run, marked as such with evidence), dimension bands + evidence, Drift Classification
  summary, full findings, advisory note

## Postconditions

None enforced — the developer may proceed to `/speckit-plan` regardless of findings. Any override
recorded here (Step 8) is carried forward by `/speckit-questmaster-check-plan` (FR-039).

## Done When

- [ ] Any pending story was relocated before assessment, or the no-story silent-exit/explicit-
      explanation branch was followed correctly for the trigger type
- [ ] Source digests computed and compared against any previous report; a mismatch was stated,
      not silently ignored
- [ ] Every outstanding accepted risk was restated or, with cited evidence, auto-resolved and
      reported as reopenable
- [ ] All 7 dimensions banded with evidence, and every requirement/scope item/assumption given
      exactly one Drift Classification, via an independent Agent tool invocation (or explicitly
      labelled SELF-ASSESSED if none was available)
- [ ] `spec-integrity.md` written in the normative decision-first order; the same content
      presented in the response
- [ ] The top Decision Worklist item (if any) was put to the developer and their response
      recorded as a Judgment Ledger entry, without blocking
